...
function wifi_deps {
    ...
    if [ "$DIST" = "Fedora" -o "$DIST" = "RedHatEnterpriseServer" -o "$DIST" = "CentOS" ]; then
        ...
        echo "$install gcc make socat psmisc xterm ssh iperf telnet ethtool help2man $pf pylint $pep ${PYPKG}-pexpect ${PYPKG}-tk"
        $install gcc make socat psmisc xterm ssh iperf telnet \
                 ethtool help2man $pf pylint $pep \
                 net-tools \
                 ${PYPKG}-pexpect ${PYPKG}-tk
        # Install pip
        echo "$install ${PYPKG}-pip || $install ${PYPKG}-pip-whl"
...
function of {
    ...
    if [ -d "$BUILD_DIR/openflow" ]; then
        echo "Removing existing openflow directory..."
        rm -rf "$BUILD_DIR/openflow"
    fi
    if [ "$DIST" = "Fedora" -o "$DIST" = "RedHatEnterpriseServer" -o "$DIST" = "CentOS" ]; then
    ...
    if [ "$DIST" = "Ubuntu" ] &&  [ `expr $RELEASE '>=' 24.04` = "1" ]; then
        git clone --depth=1 https://github.com/ramonfontes/openflow "$BUILD_DIR/openflow"
    else
        git clone --depth=1 https://github.com/mininet/openflow "$BUILD_DIR/openflow"
    fi
